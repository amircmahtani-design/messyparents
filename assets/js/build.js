...
async function main() {
  ...
  /* Bake the page's hero illustration into the markup.

     Text, the footer, the pills and the book list are all applied at build
     time; the hero was the one thing left to the browser. applyPage() in
     mpc-store.js reads pages/<id>.image from Firestore and swaps it in after
     load, which means a page whose hero was set in Studio cannot paint it
     until Firestore has answered AND the image has come down from Firebase
     through the CDN — two round trips after first paint.

     That is why /guides.html sat with an empty hero slot for a beat while the
     home page, which uses a bundled local illustration, appeared instantly. It
     was not a slower image; it was an image the page did not yet know about.

     Baking the resolved URL into src, and preloading it, puts the hero in the
     first paint like every other picture on the site. The runtime swap still
     runs and is now a no-op, because setImageSource() short-circuits when the
     element already points at the URL it was going to set. */
  function bakeHero(html, pageId) {
    const src = data.pages[pageId] && data.pages[pageId].image;
    if (!src || !/id="pageHeroImg"/.test(html)) return html;

    /* Same helper the runtime uses, so the baked URL is byte-identical to the
       one setImageSource() would have produced — which is what makes the
       runtime swap a no-op rather than a second fetch. */
    const resolved = R.img(src, 1100);

    html = html.replace(/(<img[^>]*id="pageHeroImg"[^>]*)/, (tag) => {
      let out = tag;
      out = /\ssrc="/.test(out)
        ? out.replace(/\ssrc="[^"]*"/, ' src="' + resolved + '"')
        : out + ' src="' + resolved + '"';
      out = /\sdata-src-original="/.test(out)
        ? out.replace(/\sdata-src-original="[^"]*"/, ' data-src-original="' + src + '"')
        : out + ' data-src-original="' + src + '"';
      if (!/class="[^"]*\bready\b/.test(out)) {
        out = /\sclass="/.test(out)
          ? out.replace(/\sclass="/, ' class="ready ')
          : out + ' class="ready"';
      }
      return out;
    });

    /* Preload it, so the fetch starts with the document rather than after the
       parser reaches the element. */
    const link = '<link rel="preload" as="image" href="' + resolved + '" fetchpriority="high">';
    if (!html.includes(link)) html = html.replace(/<\/head>/i, link + "\n</head>");
