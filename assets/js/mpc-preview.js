/* ============================================================================
   STUDIO LIVE PREVIEW SHIM — never loaded by a reader.

   Studio previews the real site in an iframe and pushes the draft straight in,
   so a change to a heading or an illustration appears without a reload and
   without saving. It did that by calling into MPCStore, which every public
   page used to load.

   Public pages no longer load MPCStore, because everything it did at runtime —
   the editable wording, the footer, the hero illustrations, the books list —
   is now baked into the HTML by scripts/lib/bake.js at deploy time. There is
   nothing left for a reader to apply.

   Studio still needs it, though, because a draft has not been deployed yet.
   So this is the DOM-applying half of the old mpc-store.js, lifted out
   unchanged, with the Firebase boot and the whole-collection Firestore read
   removed. Studio injects it into the preview iframe and calls it exactly as
   before.

   It is injected on demand into an iframe Studio controls. It is not
   referenced by any public page, and tests/verify.js fails the build if one
   ever starts to reference it.
   ========================================================================== */
window.MPCStore = window.MPCStore || {};
window.MPC_PAGES = window.MPC_PAGES || {};
MPCStore.pages = MPCStore.pages || {};
/* Kept so Studio call sites that await it still resolve. Nothing to wait for:
   the draft is handed in directly. */
MPCStore.ready = Promise.resolve([]);
MPCStore.pagesReady = Promise.resolve();
...
MPCStore.applyText = function (pages) {
  ...
  Array.prototype.forEach.call(nodes, function (el) {
    ...
    /* The build bakes overrides straight into the HTML and parks the shipped
       wording in data-mpc-default. So "what this resets to" is that attribute
       when it exists — otherwise clearing a box in Studio would preview the
       baked override rather than the original, and reset would look broken. */
    if (el.__mpcOriginal == null) {
      var baked = el.getAttribute("data-mpc-default");
      el.__mpcOriginal = (baked != null) ? baked : el.innerHTML;
    }
    var v = MPCStore.text[key];
  ...
  Array.prototype.forEach.call(document.querySelectorAll("[data-mpc-attr]"), function (el) {
    ...
    if (!attr || !key) return;
    if (el.__mpcOriginalAttr == null) {
      var bakedAttr = el.getAttribute("data-mpc-attr-default");
      el.__mpcOriginalAttr = (bakedAttr != null) ? bakedAttr : (el.getAttribute(attr) || "");
    }
    var v = MPCStore.text[key];
...
MPCStore.applyFooter = function (cfg) {
  ...
  [["copyright", "[data-foot-copy]"], ["note", "[data-foot-note]"]].forEach(function (pair) {
    ...
    if (el.__mpcOriginal == null) {
      var bakedFoot = el.getAttribute("data-mpc-default");
      el.__mpcOriginal = (bakedFoot != null) ? bakedFoot : el.innerHTML;
    }
