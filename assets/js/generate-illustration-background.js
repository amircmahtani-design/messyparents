...
/* Why did we not get an image?

   "No image returned from generation step." was reported identically for three
   completely different failures: the orchestrator replying in text instead of
   calling the image tool, the image tool running but returning no result
   (moderation, an internal error), and an outright refusal. Each needs a
   different response from the operator, so the reason is now dug out. */
function diagnoseMissingImage(resp) {
  const out = (resp && resp.output) || [];
    ...
    if (item.type === "image_generation_call") {
      ...
      if (status !== "completed") {
        return "the image tool ran but returned status \"" + status + "\"" +
               ...
               ". This is usually a content filter on the prompt or reference images.";
      }
  ...
  for (const item of out) {
    if (item.type === "message" && Array.isArray(item.content)) {
      for (const c of item.content) {
        if (c.type === "refusal" && c.refusal) return "the model refused: " + c.refusal;
      }
    }
  }
  ...
  if (text) {
    return "the model replied with text instead of drawing: \"" +
...
function cutoutMagenta(b64) {
  ...
  /* ---- 5) Trim the transparent border.

     The FRAME MARGIN rule tells the model to leave at least 8% of the canvas
     empty on all four sides, which is what stopped illustrations being drawn
     off the edge and looking cropped. The side effect is that the cut-out PNG
     keeps all that empty space as transparent padding.

     The hero slot uses object-fit:contain, so it scales the whole file —
     padding included — to fit. A picture with a wide transparent border
     therefore renders visibly smaller than one without, and heroes made before
     the margin rule existed sit alongside heroes made after it at noticeably
     different sizes.

     Cropping to the alpha bounding box gives every hero the same treatment:
     the artwork itself fills the frame, so they all render at a consistent
     size regardless of how much air the model left around them. A few pixels
     of padding are kept so feathered edges are not clipped. */
  ...
  if (maxX < minX || maxY < minY) {
    /* Nothing opaque survived — return the image untrimmed and let the
       transparency gate downstream fail it, rather than emitting a 0x0 PNG. */
  ...
  for (let y = 0; y < ch; y++) {
    ...
    d.copy(out.data, y * cw * 4, src, src + cw * 4);
  }

  return { b64: PNG.sync.write(out).toString("base64"), width: cw, height: ch };
}
...
function aspectRatioToSize(aspectRatio, brief) {
  ...
  /* AUTO = landscape, always.

     This used to return square unless the scene had three or more characters,
     which is why "Auto" kept producing squares — most casts are two.

     Landscape is not just the more common case, it is the one the page is
     built for. The hero slot is 460x300 with object-fit:contain, a ratio of
     1.53. A 3:2 landscape render fills 98% of it. A square fills 65% and
     leaves 160px of dead width beside it; a portrait fills 44%.

     Standardising also gives every guide page an identically sized hero, which
     is what stops the pages looking subtly mismatched as you scroll between
     them — the same consistency argument that favours square, but landed on
     the ratio that does not waste a third of the space. */
  return "1536x1024";
}
...
async function generateImage(brief, refUrls, retryNotes, userInstructions, advice, quality) {
  ...
  const call = (extraNudge) => callResponses({
    model: ORCH_MODEL,
    tools: [imageTool(size, quality)],
    input: [{ role: "user", content: [
      { type: "input_text", text: extraNudge ? (extraNudge + "\n\n" + prompt) : prompt },
      ...refUrls.map(imgInput)
    ]}]
  });

  let resp = await call(null);
  let img = extractImage(resp);

  /* The commonest cause of an empty result is the orchestrator answering in
     prose instead of calling the image tool. That is a transient miss, not a
     real failure, and one nudge almost always clears it. */
  const chattedInsteadOfDrawing =
    !img && !(resp && (resp.output || []).some(i => i.type === "image_generation_call"));

  if (chattedInsteadOfDrawing) {
    console.warn("generation: no tool call on first pass —", diagnoseMissingImage(resp));
    resp = await call(
      "Call the image_generation tool now and return an image. Do not reply with " +
      "text, do not describe the picture, do not ask questions. Draw it."
    );
    img = extractImage(resp);
  }

  if (!img) {
    const why = diagnoseMissingImage(resp);
