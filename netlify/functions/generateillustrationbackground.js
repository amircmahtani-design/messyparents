...
function imageTool(size, transparent) {
  const t = { type: "image_generation", model: "gpt-image-1", output_format: "png", size: size || "1024x1024", quality: "high" };
  if (transparent !== false) t.background = "transparent";
  return t;
}
function imgInput(url) { return { type: "input_image", image_url: url }; }

// Try transparent first; if the model refuses it, retry opaque so it never hard-fails.
async function genResponses(base, size) {
  try { return await callResponses({ ...base, tools: [imageTool(size, true)] }); }
  catch (e) {
    if (/transparent|background|not supported/i.test(String((e && e.message) || ""))) {
      return await callResponses({ ...base, tools: [imageTool(size, false)] });
    }
    throw e;
  }
}
