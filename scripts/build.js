...
async function main() {
  ...
  redirects.push("");
  redirects.push("# ---------------------------------------------------------------------");
  redirects.push("# Unknown guide slugs — must stay last.");
  redirects.push("#");
  redirects.push("# This build wrote a real file for every guide above, and Netlify serves a");
  redirects.push("# real file in preference to any non-forced rule. So anything still");
  redirects.push("# reaching this line is a guide that does not exist, and it gets a genuine");
  redirects.push("# 404 — not a styled 'not found' page returning 200, which would leave");
  redirects.push("# Google indexing dead URLs as though they were real ones.");
  redirects.push("#");
  redirects.push("# The committed fallback _redirects in the repository does the opposite:");
  redirects.push("# it rewrites to guide.html with a 200 so that every guide still renders");
  redirects.push("# if this build never ran. That file only survives when this one is not");
  redirects.push("# generated, so the two can never both apply.");
  redirects.push("/guides/*  /404.html  404");
