...
async function main() {
  ...
  redirects.push("");
  redirects.push("# Trailing-slash normalisation for the clean URLs.");
  redirects.push("#");
  redirects.push("# Written out one URL at a time rather than as");
  redirects.push("#   /guides/:slug  /guides/:slug/  301!");
  redirects.push("# because the wildcard form can redirect a URL to itself.");
  redirects.push("# Netlify matches /guides/foo/ against /guides/:slug whenever");
  redirects.push("# nothing is being served at that path, so the moment a generated");
  redirects.push("# page is missing the rule bounces the URL back to itself and the");
  redirects.push("# browser stops with ERR_TOO_MANY_REDIRECTS. A missing page is");
  redirects.push("# exactly when the site most needs to fall through to the rewrite");
  redirects.push("# at the bottom, and instead it broke hardest.");
  redirects.push("#");
  redirects.push("# Naming each source explicitly makes the loop impossible: the source");
  redirects.push("# never ends in a slash, the target always does, so they can never be");
  redirects.push("# the same string. An unknown slug falls through as intended.");
  guides.forEach(g => redirects.push(`/guides/${g.slug}  ${g.url}  301!`));
  topics.forEach(tp => redirects.push(`/topics/${tp.id}  ${S.topicUrl(tp.id)}  301!`));
  ages.forEach(a => {
    const sl = S.ageSlug(a);
    redirects.push(`/ages/${sl}  ${S.ageUrl(sl)}  301!`);
  });
  redirects.push("");
  redirects.push("# ---------------------------------------------------------------------");
